...
Можно найти пересечение графиков любых  из функций при помощи флага -root, но стоит помнить, что для этого нужно, чтобы у функции (f(x) - g(x)) первая и вторая производные не меняли знак на отрезке, где ещется пересечение. 
